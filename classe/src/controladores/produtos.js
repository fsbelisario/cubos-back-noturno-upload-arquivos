...
const cadastrarProduto = async (req, res) => {
  const { usuario } = req;
  const { nome, estoque, preco, categoria, descricao, imagem, imagem_arquivo } = req.body;
  ...
  try {
    ...
    if (!!imagem && !!imagem_arquivo) {
      const buffer = Buffer.from(imagem_arquivo, 'base64');

      const { data, error } = await supabase
        .storage
        .from(process.env.SPACES_BUCKET)
